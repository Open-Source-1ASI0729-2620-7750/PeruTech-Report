...
En esta sección se especifican los requisitos funcionales y técnicos del sistema mediante Epics y User Stories, derivados de los hallazgos cualitativos del proceso de Needfinding y del análisis del dominio del problema. La redacción sigue el enfoque de desarrollo centrado en el usuario, estructurando cada descripción bajo el estándar de rol, necesidad y beneficio. 

Asimismo, los criterios de aceptación se definen bajo la sintaxis formal de Gherkin (Given-When-Then), redactados en tercera persona y prescindiendo de detalles de interfaz gráfica (como botones, pantallas o clics), asegurando su comprobabilidad tanto en la experiencia web interactiva como en los contratos de servicio del RESTful API.
